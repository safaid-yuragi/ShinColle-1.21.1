...
public class CalcHelper
{
    ...
    {
        //normalize
        double d1 = Mth.sqrt((float) (motX * motX + motY * motY + motZ * motZ));
        ...
        double f1 = Mth.sqrt((float) (motX*motX + motZ*motZ));
    ...
    {
        return getEntityHitHeight(Minecraft.getInstance().player, target);
    ...
    public static int getEntityHitHeight(Entity host, Entity target)
    {
        ...
        {
            //calc tan(pitch)
            float x1 = (float) Math.tan(host.getXRot() * Values.N.DIV_PI_180);
            ...
            //calc distance
            float dist = host.distanceTo(target) - target.getBbWidth() * 0.5F;
            ...
            float x2 = (float) (host.getY() + host.getEyeHeight() - target.getY());
            float x = x2 - x1;

            //turn height(float) to percent(int)
            result = (int) (x / target.getBbHeight() * 100F);
        }
    ...
    public static int getEntityHitSideByClientPlayer(Entity target)
    {
        ...
        if (target != null)
        {
            float angHost = Minecraft.getInstance().player.getYHeadRot();
            float angTarget = 0F;

            if (target instanceof LivingEntity)
            {
                angTarget = ((LivingEntity) target).yBodyRot;
            }
            else
            {
                angTarget = target.getYRot();
            }
    ...
    public static int getEntityHitSide(Entity host, Entity target)
    {
        ...
        {
            result = (int) ((host.getYRot() % 360 - target.getYRot() % 360) % 360);
    ...
    public static float[] rotateXZByLook(float x, float y, float lookX, float lookY, float scale)
    {
        ...
        float cosZdeg = Mth.cos(degree[0]);
        float sinZdeg = Mth.sin(degree[0]);
        float[] newPos = new float[] {0F, 0F};
    ...
    public static Dist4d getDistanceFromA2B(BlockPos host, BlockPos target)
    {
        if (host != null && target != null)
        {
            return getDistanceFromA2B(new Vec3(host.getX(), host.getY(), host.getZ()),
    ...
    public static Dist4d getDistanceFromA2B(Object host, Entity target)
    {
        ...
        {
            return getDistanceFromA2B(new Vec3(((BlockEntity) host).getBlockPos().getX() + 0.5D,
                                                ((BlockEntity) host).getBlockPos().getY() + 0.5D,
                                                ((BlockEntity) host).getBlockPos().getZ() + 0.5D),
                                      target.position());
        }
        else
        {
    ...
    public static Dist4d getDistanceFromA2B(Entity host, Entity target)
    {
        if (host != null && target != null)
        {
            return getDistanceFromA2B(host.position(), target.position());
        }
     ...
     * return: "normalized" distance vector [x, y, z, distance]
     */
    public static Dist4d getDistanceFromA2B(Vec3 host, Vec3 target)
    {
        if (host != null && target != null)
        {
     ...
     * return: "normalized" distance vector [x, y, z, distance]
     */
    public static Vec3 getUnitVectorFromA2B(Vec3 from, Vec3 to)
    {
