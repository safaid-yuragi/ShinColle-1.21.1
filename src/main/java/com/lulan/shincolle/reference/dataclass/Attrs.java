 ...
 * 
 * 若要新增新屬性, 需修改ID.Attrs, Attrs.AttrsLength, ShinColleConfig.limitShipAttrs
 */
public class Attrs
{
	...
	public static void checkAttrsLimit(float[] data)
	{
		...
		{
			if (ShinColleConfig.limitShipAttrs[i] >= 0D && data[i] > ShinColleConfig.limitShipAttrs[i])
			{
				data[i] = (float) ShinColleConfig.limitShipAttrs[i];
			}
		}
	...
	public boolean addAttrsBonusRandom(Random rand)
	{
		...
		if (this.AttrsBonus[bonusChoose] < ShinColleConfig.modernLimit)
		{
		...
		else
		{
			...
			for (int i = 0; i < 6; ++i)
			{
				if (this.AttrsBonus[i] < ShinColleConfig.modernLimit)
				{
					this.AttrsBonus[i] += 1;
					return true;
				}
